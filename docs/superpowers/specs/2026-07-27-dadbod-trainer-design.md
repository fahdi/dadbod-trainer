...
## Structure
- `index.html` — diary homepage. Lists sessions newest-first as cards (date, title, quick stats), each linking to its session page.
- `sessions/session-N.html` — one page per workout. Contains:
  - The clean session card: Round / Slot / Exercise / Muscle / Target / Note, exactly as structured in the source Excel file.
  - A collapsible "How this was built" section with the coaching transcript for that session: original Slack messages, Claude's draft attempts (including the wrong ones), Brian's verbatim corrections, and the "RULE LEARNED" callouts, closing with the consolidated rule list.
